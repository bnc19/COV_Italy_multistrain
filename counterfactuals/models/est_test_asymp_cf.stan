...
// Simulate Italy --------------------------------------------------------------
 for (t in 1:(n_ts_it)){
   ...
   IA_it[t,i] + (1-mu) * (1-delta_it[t,i]) * sigma2 * PS_it[t,i] - gamma2 * IA_it[t,i];

   
   Q_it[t+1] = 
   Q_it[t] + (delta_it[t,1] * (1-mu) * sigma2 * PS_it[t,1]) 
   + (delta_it[t,2] * (1-mu) * sigma2 * sum(PS_it[t,2:4])) 
   + mu * sigma2 * sum(PS_it[t,]) - gamma2 * Q_it[t];
  ...
  ( (delta_it[t,i]*(1-mu)*sigma2*PS_it[t,i]) + (mu * sigma2 * PS_it[t,i]) )  ;
 }
...
// Simulate Veneto -------------------------------------------------------------
 for (t in 1:(n_ts_ven)){
   ...
   IA_ven[t,i] +(1-delta_ven[t,i]) * (1-mu) * sigma2 * PS_ven[t,i] - gamma2 * IA_ven[t,i];
   
   Q_ven[t+1] = 
   Q_ven[t] + (delta_ven[t,1] * (1-mu) * sigma2 * PS_ven[t,1]) 
   + (delta_ven[t,2] * (1-mu) * sigma2 * sum(PS_ven[t,2:4]))
   + mu * sigma2 * sum(PS_ven[t,]) - gamma2 * Q_ven[t];
  ...
  ( (delta_ven[t,i]*(1-mu) *sigma2*PS_ven[t,i]) + (mu * sigma2 *PS_ven[t,i]) )   ;
 }
