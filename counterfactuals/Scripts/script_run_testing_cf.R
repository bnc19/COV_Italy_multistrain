################################################################################
# Script to run the counterfactual testing scenarios for Veneto (assuming      #
# proportion of antigen and molecular tests conducted in Veneto was the same   #
# as in the rest of Italy over the study period; assuming a molecular test     #
# followed a negative antigen test; assuming an antigen only testing strategy  #
# and an antigen test sensitivity of 68.9%; assuming an antigen only testing   #
# strategy and an antigen test sensitivity of 87.5%. Outputs the data          #
...
model_posts_cf4 = sapply(1:nrow(posterior_samples), function(i) {
  replicate_rstan_fixed(
    model = molecular_after_antigen_neg_model,
    posterior_sample_row = posterior_samples[i,],
    scale_time_step = 2,
    start_date_veneto =  "01-05-2020",
    time_seed_M_veneto = "01-08-2020"
