...
test1Data = data.frame(read_excel(SupplMaterial, 
                                sheet="ANCOV or DNCOV"))
names(test1Data)[3:4] = c("AgeRange", "HospitalWard")

test1Data$DNCOV[test1Data$DNCOV == "-"] = NA
test1Data$ANCOV[test1Data$ANCOV == "-"] = NA
...
-------------------
```{r}
TPData = test2Data_20[with(test2Data_20, (ANCOV=="positive") | 
                          (DNCOV=="positive")),]
TPData = TPData[!((TPData$DNCOV=="negative") & 
                        (TPData$ANCOV=="positive")),]

TPData_tab = with(test2Data_20, table(DNCOV, ANCOV))
...
allData = data.frame(
  ...
  DNCOV = factor(c(test2Data$DNCOV,test1Data$DNCOV),
                 labels=c("Negative", "Positive")),
  ANCOV = factor(c(test2Data$ANCOV,test1Data$ANCOV),
                 labels=c("Negative", "Positive")),
...
allData_20 = data.frame(
  ...
  DNCOV = factor(c(test2Data_20$DNCOV,test1Data_20$DNCOV),
                 labels=c("Negative", "Positive")),
  ANCOV = factor(c(test2Data_20$ANCOV,test1Data_20$ANCOV),
                 labels=c("Negative", "Positive")),
...
```{r}
pr = with(test2Data_20, mean(DNCOV=="positive"))
...
pr = with(test2Data_20[test2Data_20$CtSgene < 33,], 
          sum(DNCOV=="positive", na.rm=TRUE))/nrow(test2Data_20[test2Data_20$CtSgene < 33,])
...
pr = with(test2Data_20[test2Data_20$CtORF1gene < 33,], 
          sum(DNCOV=="positive", na.rm=TRUE))/nrow(test2Data_20[test2Data_20$CtORF1gene < 33,])
...
{   
  ...
  sensSgene[idx] = mean(tmp$ANCOV == "positive")
  CtScounts[idx,] = c(sum(tmp$ANCOV == "positive"), 
                      nrow(tmp)-sum(tmp$ANCOV == "positive"))
}
...
{   
  ...
  sensORF1gene[idx] = mean(tmp$ANCOV == "positive")
  CtORF1counts[idx,] = c(sum(tmp$ANCOV == "positive"), 
                         nrow(tmp)-sum(tmp$ANCOV == "positive"))
}
...
predORF1_tab = pred_tmp
```

**Figure 2**

```{r, fig.align="center", fig.cap="Figure2: Sensitivity estimates (solid line) of ANCOV test for increasing Ct values of the S gene (left panel) and Ct ORF1 (right panel) gene as obtained from a logistic regression model. The dashed lines give the 95% confidence band for the fitted regression line. The dotted lines represent the 95% band for the fitted sensitivities."}
...
```{r, fig.align="center", fig.cap="Distribution of Ct values for concordant DNCOV+/ANCOV+ and discordant DNCOV+ / ANCOV- test results. Top row: Boxplots of Ct values for the S (left) and ORF1 (right) gene. Dotted line at Ct = 33 shows the critical value of sensitivity (95%) declared for the antigen test above which there is an expected drop and an unreliable result. DNCOV+/ANCOV+ (grey box), DNCOV+/ANCOV- below sensitive threshold (yellow box) DNCOV+/ANCOV- above sensitive threshold (blue box). Bottom row: Nonparametric density estimation for Ct values of the S (left) and ORF1(right) gene, with superimposed the two densities fitted using a two-component Gaussian mixture model. DNCOV+/ANCOV- below sensitive threshold (yellow line) DNCOV+/ANCOV- above sensitive threshold (blue line)."}
...
```{r, fig.align="center", fig.cap="Monthly prevalence (percentage over total sequences) of N variants generating discordant DNCOV+ / ANCOV- (left panels) and concordant DNCOV+ / ANCOV+ (right panels) result in molecular and antigen swab. Shaded areas show the 95% confidence bands for each different region. No sequence data are available for Veneto region in the months of January, February, July, and September."}
...
kbl(t(mat[,-1]), align = "c", row.names = TRUE, 
    ...
    caption = "Overall sensitivity/specificity and PPV/NPV for ANCOV test") %>%
  kable_classic() %>%
...
kbl(pred_tab, align="c", col.names=c("< value", "sensitivity", "lower 95% confidence bound", "upper 95% confidence bound", "< value", "sensitivity", "lower 95% confidence bound", "upper 95% confidence bound"), caption="Sensitivity estimates of ANCOV test for successively increasing Ct S gene values (on the left) and Ct ORF1 gene values (on the right) as obtained form a logistic regression model.") %>%
  kable_classic() %>%
  add_header_above(c("Ct S gene" = 4, "Ct ORF1 gene"  = 4))
```
...
with(allData_20, barplot(prop.table(table(which, Age), mar=1), 
                      beside=TRUE, legend.text=c("ANCOV", "both", "DNCOV"),
...
with(test1Data, barplot(prop.table(table(which, Age), mar=2), 
                      beside=FALSE, legend.text=c("ANCOV", "DNCOV"),
...
```{r}
with(allData_20[!is.na(allData_20$DNCOV),], table(type, DNCOV))
with(allData_20[!is.na(allData_20$DNCOV),], 
     round(prop.table(table(type, DNCOV), mar=1), 3))
with(allData_20[!is.na(allData_20$DNCOV),], 
     barplot(prop.table(table(type, DNCOV), mar=1), beside=TRUE,
             legend.text=c("both","either")))
with(allData_20[!is.na(allData_20$DNCOV),], prop.test(table(type, DNCOV)))
```
...
```{r}
with(allData_20[!is.na(allData_20$ANCOV),], table(type, ANCOV))
with(allData_20[!is.na(allData_20$ANCOV),], 
     round(prop.table(table(type, ANCOV), mar=1), 3))
with(allData_20[!is.na(allData_20$ANCOV),], 
     barplot(prop.table(table(type, ANCOV), mar=1), beside=TRUE,
             legend.text=c("both","either")))
with(allData_20[!is.na(allData_20$ANCOV),], prop.test(table(type, ANCOV)))
```
...
tmp[(allData_20$type == "either") & (!is.na(allData_20$DNCOV)] = "DNCOV"
tmp[(allData_20$type == "either") & (!is.na(allData_20$ANCOV)] = "ANCOV"
