run_SEIR_stan_Italy_sens = function(Al_data,
  ...
  daily_reported_incidence_i = daily_reported_incidence[(length(daily_reported_incidence) -
                                                           n_days + 1):length(daily_reported_incidence)]
  ...
  average_daily_vaccination_i = average_daily_vaccination[(length(average_daily_vaccination) -
                                                             n_months + 1):length(average_daily_vaccination)]
  ...
  model_data = list(
    ...
    x_r_data = c(average_daily_vaccination_i, sigma, gamma,  phi_PCR, phi_Ag),
    x_i_data = c(
      n_months,
      index_1st_month,
      monthly_PCR_i,
  ...
  param_pairs = mcmc_pairs(SEIR_fit_posterior, pars = c(pars))
