...
// Simulate Italy --------------------------------------------------------------
 for (t in 1:(n_ts_it)){
   ...
   IS_it[t,i] + (1-delta_it[t,i]) * mu * sigma * PS_it[t,i] - gamma * IS_it[t,i];
   
   Q_it[t+1] = 
   Q_it[t] + (delta_it[t,1] * mu * sigma * PS_it[t,1]) 
   + (delta_it[t,2] * mu * sigma * sum(PS_it[t,2:4])) - gamma * Q_it[t];
   
   R_it[t+1] = 
   R_it[t] + gamma * Q_it[t] + gamma * sum(IS_it[t,])  
...
// Simulate Veneto -------------------------------------------------------------
 for (t in 1:(n_ts_ven)){
   ...
   IS_ven[t,i] + (1-delta_ven[t,i]) * mu * sigma * PS_ven[t,i] - gamma * IS_ven[t,i];
   
   Q_ven[t+1] = 
   Q_ven[t] + (delta_ven[t,1] * mu * sigma * PS_ven[t,1]) 
   + (delta_ven[t,2] * mu * sigma * sum(PS_ven[t,2:4])) - gamma * Q_ven[t];
   
   R_ven[t+1] = 
   R_ven[t] + gamma * Q_ven[t] + gamma * sum(IS_ven[t,]) 
