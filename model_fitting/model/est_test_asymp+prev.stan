...
parameters {
  ...
  real<lower = gamma>         tau ;     // testing rate once infectious 
...
// Simulate Italy --------------------------------------------------------------
 for (t in 1:(n_ts_it)){
   ...
   E_it[time_seed_M_it,1] = I0_it[1];
   E_it[time_seed_alpha_it,4] = I0_it[4];
   ...
   beta2_it[t,i] * (IP_it[t,i] + IA_it[t,i]+ IS_it[t,i]) / S0_it;
   ...
   for(i in 1:n_var) IP_it[t+1,i] = 
   IP_it[t,i] + epsilon * E_it[t,i] - sigma * IP_it[t,i];
   
   
   for(i in 1:n_var) IA_it[t+1,i] = 
   IA_it[t,i] + (1-mu) * sigma * IP_it[t,i] 
   - (1-delta_it[t,i]) * gamma * IA_it[t,i] 
   - delta_it[t,i] * tau * IA_it[t,i] ;
   
   for(i in 1:n_var) IS_it[t+1,i] = 
   IS_it[t,i] + mu * sigma * IP_it[t,i] - tau * IS_it[t,i]; 

   
   Q_it[t+1] = 
   Q_it[t] + tau * sum(IS_it[t,]) +  delta_it[t,1] * tau * IA_it[t,1] 
   + delta_it[t,2] * tau * sum(IA_it[t,2:4]) - gamma * Q_it[t];
   
   R_it[t+1] = 
   R_it[t] + gamma * Q_it[t] + (1-delta_it[t,1]) * gamma * IA_it[t,1] 
   + (1-delta_it[t,2]) * gamma * sum(IA_it[t,2:4]) + vac_it[t] * S_it[t];
   
   
   for(i in 1:n_var) daily_incidence_it[t,i] = 
  (tau * sum(IS_it[t,]) +  delta_it[t,1] * tau * IA_it[t,1] 
   + delta_it[t,2] * tau * sum(IA_it[t,2:4]) ) / S0_it * 100000  ;
 }
...
// Simulate Veneto -------------------------------------------------------------
  for (t in 1:(n_ts_ven)){
   ...
   E_ven[time_seed_M_ven,1] = I0_ven[1];
   E_ven[time_seed_alpha_ven,4] = I0_ven[4];
   ...
   beta2_ven[t,i] * (IP_ven[t,i] + IA_ven[t,i]+ IS_ven[t,i]) / S0_ven;
   ...
   for(i in 1:n_var) IP_ven[t+1,i] = 
   IP_ven[t,i] + epsilon * E_ven[t,i] - sigma * IP_ven[t,i];
   
   
   for(i in 1:n_var) IA_ven[t+1,i] = 
   IA_ven[t,i] + (1-mu) * sigma * IP_ven[t,i] 
   - (1-delta_ven[t,i]) * gamma * IA_ven[t,i] 
   - delta_ven[t,i] * tau * IA_ven[t,i] ;
   
   for(i in 1:n_var) IS_ven[t+1,i] = 
   IS_ven[t,i] + mu * sigma * IP_ven[t,i] - tau * IS_ven[t,i]; 

   
   Q_ven[t+1] = 
   Q_ven[t] + tau * sum(IS_ven[t,]) +  delta_ven[t,1] * tau * IA_ven[t,1] 
   + delta_ven[t,2] * tau * sum(IA_ven[t,2:4]) - gamma * Q_ven[t];
   
   R_ven[t+1] = 
   R_ven[t] + gamma * Q_ven[t] + (1-delta_ven[t,1]) * gamma * IA_ven[t,1] 
   + (1-delta_ven[t,2]) * gamma * sum(IA_ven[t,2:4]) + vac_ven[t] * S_ven[t];
   
   
   for(i in 1:n_var) daily_incidence_ven[t,i] = 
  (tau * sum(IS_ven[t,]) +  delta_ven[t,1] * tau * IA_ven[t,1] 
   + delta_ven[t,2] * tau * sum(IA_ven[t,2:4]) ) / S0_ven * 100000  ;
 }
