...
parameters {
  real            L_beta[n_var];      // transmission parameter 
  real            L_I0_ven[n_var];    // seed
  real            L_I0_it[n_var];     // seed
  real<upper = 0> L_rho_it;           // probability of reporting 
  real<upper = 0> L_omega[2];         // reduction in transmission
  real            L_k;                // overdispersion parameter 
  real<upper = 0> L_alpha; 
}   
...
transformed parameters{
 ...
 real rho_it = exp(L_rho_it) ;
 real k = exp(L_k) ;
 
 real I0_it_M = exp(L_I0_it[1]) ;
 real I0_it_A = exp(L_I0_it[2]) ;
 real I0_it_O = exp(L_I0_it[3]) ;
 real I0_it_Al = exp(L_I0_it[4]) ;
 
 real I0_ven_M = exp(L_I0_ven[1]) ;
 real I0_ven_A = exp(L_I0_ven[2]) ;
 real I0_ven_O = exp(L_I0_ven[3]) ;
 real I0_ven_Al = exp(L_I0_ven[4]) ;
 
 real alpha = exp(L_alpha);
...
// Define Veneto param ---------------------------------------------------------
  ...
  // Scale beta 
  for(i in 1:n_var) beta[i] = exp(L_beta[i]) ;
  for(i in 1:n_var) beta2[i] = beta[i] / scale_time_step;
  for(i in 1:2) omega[i] = exp(L_omega[i]) ; 
...
// Simulate Italy --------------------------------------------------------------
 for (t in 1:(n_ts_it)){
   ...
   beta2_it[t,i] * (PS_it[t,i] + IA_it[t,i] + (alpha * IS_it[t,i])) / S0_it;
   ...
   (delta_it[t,i]*mu*sigma2*PS_it[t,i]) / S0_it * 100000  ;
 }
...
// Simulate Veneto -------------------------------------------------------------
 for (t in 1:(n_ts_ven)){
   ...
   beta2_ven[t,i] * (PS_ven[t,i] + IA_ven[t,i] + (alpha * IS_ven[t,i])) / S0_ven;
   ...
   (delta_ven[t,i]*mu*sigma2*PS_ven[t,i]) / S0_ven * 100000  ;
 }
...
// priors ----------------------------------------------------------------------
  ...
  L_alpha  ~ normal(-0.4,0.9);
  L_beta    ~ normal(0,0.5);
  L_rho_it  ~ normal(-0.4,0.3);
  L_omega   ~ normal(-0.4,0.3);
  L_I0_it   ~ normal(2.5,1);
  L_I0_ven  ~ normal(1.5,1);
  L_k       ~ normal(-1,1);
