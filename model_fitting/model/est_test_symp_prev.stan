...
// Simulate Italy --------------------------------------------------------------
 for (t in 1:(n_ts_it)){
   ...
   IA_it[t,i] + (1-mu) * sigma * PS_it[t,i] -  gamma * IA_it[t,i];

 for(i in 1:n_var) IS_it[t+1,i] = 
   IS_it[t,i] + mu * sigma * PS_it[t,i] - (delta_it[t,1] * tau * IS_it[t,1] )
   - (delta_it[t,2] * tau * sum(IS_it[t,2:4])) - ( (1-delta_it[t,1]) * gamma * IS_it[t,1]) 
   - ((1-delta_it[t,2]) * gamma * sum(IS_it[t,2:4])) ;

   
   Q_it[t+1] = 
   Q_it[t] + (delta_it[t,1] * tau * IS_it[t,1])
   + (delta_it[t,2] * tau * sum(IS_it[t,2:4])) - gamma * Q_it[t];
   
   R_it[t+1] = 
   R_it[t] + gamma * Q_it[t] + gamma * sum(IA_it[t,]) + ((1-delta_it[t,1] )* gamma * IS_it[t,1])
   + (1-delta_it[t,2] * gamma * sum(IS_it[t,2:4])) + vac_it[t] * S_it[t];
   
   
   for(i in 1:n_var) incidence_it[t,i] =  delta_it[t,i] * tau * IS_it[t,i] / S0_it * 100000  ;
 }
...
// Simulate Veneto -------------------------------------------------------------
   ...
   IA_ven[t,i] + (1-mu) * sigma * PS_ven[t,i] -  gamma * IA_ven[t,i];

 for(i in 1:n_var) IS_ven[t+1,i] = 
   IS_ven[t,i] + mu * sigma * PS_ven[t,i] - (delta_ven[t,1] * tau * IS_ven[t,1] )
   - (delta_ven[t,2] * tau * sum(IS_ven[t,2:4])) - ( (1-delta_ven[t,1]) * gamma * IS_ven[t,1]) 
   - ((1-delta_ven[t,2]) * gamma * sum(IS_ven[t,2:4])) ;

   
   Q_ven[t+1] = 
   Q_ven[t] + (delta_ven[t,1] * tau * IS_ven[t,1])
   + (delta_ven[t,2] * tau * sum(IS_ven[t,2:4])) - gamma * Q_ven[t];
   
   R_ven[t+1] = 
   R_ven[t] + gamma * Q_ven[t] + gamma * sum(IA_ven[t,]) + ((1-delta_ven[t,1] )* gamma * IS_ven[t,1])
   + (1-delta_ven[t,2] * gamma * sum(IS_ven[t,2:4])) + vac_ven[t] * S_ven[t];
   
   
   for(i in 1:n_var) incidence_ven[t,i] =  delta_ven[t,i] * tau * IS_ven[t,i] / S0_ven * 100000  ;
 }
