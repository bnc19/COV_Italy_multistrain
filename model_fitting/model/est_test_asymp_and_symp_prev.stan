...
// Simulate Italy --------------------------------------------------------------
 for (t in 1:(n_ts_it)){
   ...
   I_it[time_seed_M_it+1,1] = I0_it[1];
   I_it[time_seed_alpha_it+1,4] = I0_it[4];
   ...
   beta2_it[t,i] * (PS_it[t,i] + I_it[t,i] ) / S0_it;
   ...
   I_it[t,i] + sigma * PS_it[t,i] - (delta_it[t,1] * tau * I_it[t,1] )
   - (delta_it[t,2] * tau * sum(I_it[t,2:4])) 
   - ((1-delta_it[t,1]) * gamma * I_it[t,1])  
   - ((1-delta_it[t,2]) * gamma * sum(I_it[t,2:4])); 


   
   Q_it[t+1] =  Q_it[t] + (delta_it[t,1] * tau * I_it[t,1])
   + (delta_it[t,2] * tau * sum(I_it[t,2:4])) - gamma * Q_it[t];
   
   R_it[t+1] = 
   R_it[t] + gamma * Q_it[t] + ((1-delta_it[t,1]) * gamma * I_it[t,1])  
   + ((1-delta_it[t,2]) * gamma * sum(I_it[t,2:4])) +  vac_it[t] * S_it[t];
   

   for(i in 1:n_var) incidence_it[t,i] =  delta_it[t,i] * tau * I_it[t,i] / S0_it * 100000  ;
 }
...
// Simulate Veneto -------------------------------------------------------------
   ...
   beta2_ven[t,i] * (PS_ven[t,i] + I_ven[t,i] ) / S0_ven;
   ...
   I_ven[t,i] + sigma * PS_ven[t,i] - (delta_ven[t,1] * tau * I_ven[t,1] )
   - (delta_ven[t,2] * tau * sum(I_ven[t,2:4])) 
   - ((1-delta_ven[t,1]) * gamma * I_ven[t,1])  
   - ((1-delta_ven[t,2]) * gamma * sum(I_ven[t,2:4])); 


   
   Q_ven[t+1] =  Q_ven[t] + (delta_ven[t,1] * tau * I_ven[t,1])
   + (delta_ven[t,2] * tau * sum(I_ven[t,2:4])) - gamma * Q_ven[t];
   
   R_ven[t+1] = 
   R_ven[t] + gamma * Q_ven[t] + ((1-delta_ven[t,1]) * gamma * I_ven[t,1])  
   + ((1-delta_ven[t,2]) * gamma * sum(I_ven[t,2:4])) +  vac_ven[t] * S_ven[t];
   

   for(i in 1:n_var) incidence_ven[t,i] =  delta_ven[t,i] * tau * I_ven[t,i] / S0_ven * 100000  ;
 }
