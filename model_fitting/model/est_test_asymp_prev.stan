...
// Simulate Italy --------------------------------------------------------------
 for (t in 1:(n_ts_it)){
   ...
   IA_it[t,i] + (1-mu) * sigma * PS_it[t,i] - 
   delta_it[t,i] * tau * IA_it[t,i] - (1-delta_it[t,i]) * gamma * IA_it[t,i];

 for(i in 1:n_var) IS_it[t+1,i] = 
   IS_it[t,i] + mu * sigma * PS_it[t,i] - tau * IS_it[t,i];
   ...
   for(i in 1:n_var) incidence_it[t,i] = tau * IS_it[t,i] + delta_it[t,i] * tau * IA_it[t,i] / S0_it * 100000  ;
 }
...
// Simulate Veneto -------------------------------------------------------------
  for (t in 1:(n_ts_ven)){
   ...
   delta_ven[t,i] * tau * IA_ven[t,i] - (1-delta_ven[t,i]) * gamma * IA_ven[t,i];

 for(i in 1:n_var) IS_ven[t+1,i] = 
   IS_ven[t,i] + mu * sigma * PS_ven[t,i] - tau * IS_ven[t,i];
   ...
   for(i in 1:n_var) incidence_ven[t,i] = tau * IS_ven[t,i] + delta_ven[t,i] * tau * IA_ven[t,i] / S0_ven * 100000  ;
  }  
