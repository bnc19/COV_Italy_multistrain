...
// Simulate Italy --------------------------------------------------------------
 for (t in 1:(n_ts_it)){
   ...
   R_it[t+1] = 
   R_it[t] + gamma2 * Q_it[t] + gamma2 * sum(IA_it[t,]) + gamma2 * sum(IS_it[t,]) + VE *  vac_it[t] * S_it[t];
   
  
   for(i in 1:n_var) incidence_it[t,i] = 
  ( (deltaA_it[t,i]*(1-mu)*sigma2*PS_it[t,i]) + (deltaS_it[t,i] * mu * sigma2 * PS_it[t,i]) ) / S0_it * 100000  ;
 }
...
// Simulate Veneto -------------------------------------------------------------
 for (t in 1:(n_ts_ven)){
   ...
   R_ven[t+1] = 
   R_ven[t] + gamma2 * Q_ven[t] + gamma2 * sum(IA_ven[t,]) + gamma2 * sum(IS_ven[t,]) + VE *  vac_ven[t] * S_ven[t];
   
  
   for(i in 1:n_var) incidence_ven[t,i] = 
  ( (deltaA_ven[t,i]*(1-mu)*sigma2*PS_ven[t,i]) + (deltaS_ven[t,i] * mu * sigma2 * PS_ven[t,i]) ) / S0_ven * 100000  ;
 }
