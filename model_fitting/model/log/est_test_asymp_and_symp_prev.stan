...
parameters {
  ...
  real<lower = 0> L_I0_ven[n_var];    // seed
  real<upper = 0> L_rho_ven;      // probability of reporting 
  real<lower = 0> L_I0_it[n_var];    // seed
  real<upper = 0> L_rho_it;      // probability of reporting 
  real<upper = 0> L_omega[4]; // reduction in transmission
  real            L_k;        // overdispersion parameter 
  real<lower = log(gamma)>  L_tau;
...
// Define Veneto param ---------------------------------------------------------
  ...
  for(i in 1:n_var) I0_ven[i] = exp(L_I0_ven[i]) ;
  for(i in 1:n_var) I0_it[i] = exp(L_I0_it[i]) ;
  for(i in 1:4) omega[i] = exp(L_omega[i]) ;
...
// priors ----------------------------------------------------------------------
  ...
  L_beta    ~ normal(0,.8);
  L_rho_it  ~ normal(-.6,.2);
  L_rho_ven ~ normal(-.6,.2);
  L_omega   ~ normal(-.6,.2);
  L_I0_it   ~ normal(1,1.2);
  L_I0_ven  ~ normal(1,1.2);
  L_k       ~ normal(0,.5);
  L_tau     ~ normal(1,.5);
}
